        ...
        <div className="lg:col-span-8 space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
             ...
             <div className="md:col-span-7 flex flex-col gap-4">
               <div className="grid grid-cols-2 gap-3 h-full">
                  <StatCard label="Current" value={`${stats.currentLevelLiters.toFixed(0)}L`} subValue={`${(stats.currentLevelLiters / LITERS_PER_CM).toFixed(1)} cm`} icon={<Droplets size={18} className="text-orange-500" />} color="purple" />
                  <StatCard label="Total Cost" value={`£${stats.totalSpent.toLocaleString()}`} subValue="All deliveries" icon={<TrendingUp size={18} className="text-violet-500" />} color="orange" />
                  <StatCard label="Daily Avg" value={`${stats.avgDailyUsage.toFixed(1)}L`} subValue="Est. rate" icon={<History size={18} className="text-orange-500" />} color="purple" />
                  <StatCard label="Heat Days" value={`${Math.round(stats.estimatedDaysRemaining)}`} subValue="Remaining" icon={<Sparkles size={18} className="text-violet-500" />} color="orange" />
               </div>
             </div>
          </div>

          <div className="glass-panel p-5 rounded-2xl orange-glow relative overflow-hidden">
            <h2 className="text-xs font-bold flex items-center gap-2 mb-3 text-orange-400 uppercase tracking-widest"><Sparkles size={14} /> Consumption Intelligence</h2>
          ...
          <div className="glass-panel p-6 rounded-2xl purple-glow">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-sm font-bold flex items-center gap-2 uppercase tracking-widest text-violet-400"><LayoutDashboard size={16} /> Usage Trend</h2>
            </div>
            <div className="h-[200px] md:h-[280px] w-full">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={chartData}>
                  ...
                  <Area type="monotone" dataKey="liters" stroke={COLORS.purple} fillOpacity={1} fill="url(#colorLiters)" strokeWidth={3} />
                </AreaChart>
              </ResponsiveContainer>
            </div>
          </div>
        ...
        <div className="lg:col-span-4 flex flex-col gap-6">
          <div className="glass-panel rounded-2xl flex-1 flex flex-col min-h-[400px]">
            <div className="p-4 border-b border-slate-800 flex justify-between items-center">
              <h2 className="text-xs font-bold flex items-center gap-2 uppercase tracking-widest text-violet-400"><History size={16} /> Audit Log</h2>
            ...
            <div className="overflow-y-auto max-h-[500px] lg:max-h-none flex-1 p-3 space-y-3">
              {entries.length === 0 ? (
                <div className="text-center py-20 text-slate-600"><p className="text-sm">No activity recorded</p></div>
              ) : (
          ...
          <div className="relative w-full max-w-md bg-slate-900 border-t md:border border-slate-800 rounded-t-3xl md:rounded-3xl shadow-2xl overflow-hidden pb-10 md:pb-0">
            <div className="p-6">
              <div className="flex justify-between items-center mb-8">
                ...
                <button onClick={() => setIsModalOpen(false)} className="bg-slate-800 p-2 rounded-full text-slate-400"><X size={20} /></button>
              </div>
              <div className="space-y-5 text-slate-50">
                <div>
                  ...
                  <input type="date" value={date} onChange={e => setDate(e.target.value)} className="w-full bg-slate-800 border border-slate-700 rounded-xl px-4 py-3.5 text-white focus:outline-none" />
                </div>
                {modalType === EntryType.READING ? (
                  <div>
                    ...
                    <div className="relative">
                      <input type="number" value={levelCm} placeholder="0" max={100} onChange={e => setLevelCm(e.target.value)} className="w-full bg-slate-800 border border-slate-700 rounded-xl px-4 py-3.5 text-3xl font-bold text-white focus:outline-none" />
                  ...
                  <>
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        ...
                        <input type="number" value={liters} placeholder="500" onChange={e => setLiters(e.target.value)} className="w-full bg-slate-800 border border-slate-700 rounded-xl px-4 py-3.5 text-white font-bold focus:outline-none" />
                      </div>
                      <div>
                        <label className="block text-[10px] font-bold text-slate-500 mb-2 uppercase tracking-widest">Total £</label>
                        <input type="number" value={cost} placeholder="0.00" onChange={e => setCost(e.target.value)} className="w-full bg-slate-800 border border-slate-700 rounded-xl px-4 py-3.5 text-white font-bold focus:outline-none" />
                      </div>
                    </div>
                    <div>
                      <label className="block text-[10px] font-bold text-slate-500 mb-2 uppercase tracking-widest">Delivery Note</label>
                      <textarea value={note} placeholder="Supplier name or delivery notes..." onChange={e => setNote(e.target.value)} className="w-full bg-slate-800 border border-slate-700 rounded-xl px-4 py-3 text-white focus:outline-none h-24 resize-none" />
                    </div>
                  </>
                )}
                <button onClick={addEntry} className={`w-full py-4 rounded-2xl font-black text-lg ${modalType === EntryType.READING ? 'bg-violet-600' : 'bg-orange-600'} text-white`}>Confirm & Save</button>
              </div>
            </div>
          </div>
        </div>
...
const OilTank: React.FC<{ levelCm: number }> = ({ levelCm }) => {
  ...
  return (
    <div className="relative w-36 h-56 md:w-40 md:h-64">
      <div className="absolute inset-0 bg-green-950 border-[6px] border-green-900 rounded-[2rem] overflow-hidden flex flex-col justify-end">
        <div className="bg-gradient-to-t from-green-600 to-green-400 w-full transition-all duration-1000 ease-out" style={{ height: `${percentage}%` }} />
      </div>
      <div className="absolute -right-8 inset-y-0 flex flex-col justify-between text-[8px] text-slate-600 font-black py-4"><span>100</span><span>75</span><span>50</span><span>25</span><span>0</span></div>
    </div>
  );
};
...
const StatCard: React.FC<{ label: string, value: string, subValue: string, icon: React.ReactNode, color: 'purple' | 'orange' }> = ({ label, value, subValue, icon, color }) => (
  <div className={`glass-panel p-3.5 rounded-2xl flex flex-col justify-between ${color === 'purple' ? 'border-violet-500/20' : 'border-orange-500/20'}`}>
    <div className={`w-8 h-8 rounded-lg flex items-center justify-center mb-3 ${color === 'purple' ? 'bg-violet-500/10 text-violet-400' : 'bg-orange-500/10 text-orange-400'}`}>{icon}</div>
    <div>
      <p className="text-[10px] text-slate-500 font-bold uppercase tracking-wider">{label}</p>
      <p className={`text-xl font-black mt-0.5 ${color === 'purple' ? 'text-violet-400' : 'text-orange-400'}`}>{value}</p>
...
const HistoryItem: React.FC<{ entry: OilEntry, onDelete: (id: string) => void }> = ({ entry, onDelete }) => (
  <div className="p-4 bg-slate-900/40 border border-slate-800/50 rounded-2xl group transition-all">
    <div className="flex justify-between items-start mb-2">
      <div className="flex items-center gap-3">
        <div className={`p-2.5 rounded-xl ${entry.type === EntryType.READING ? 'bg-violet-500/10' : 'bg-orange-500/10'}`}>
          {entry.type === EntryType.READING ? <Droplets size={14} className="text-violet-400" /> : <Truck size={14} className="text-orange-400" />}
        </div>
        <div>
          <p className="text-[9px] text-slate-500 font-black uppercase tracking-widest">{new Date(entry.date).toLocaleDateString('en-GB', { day: 'numeric', month: 'short' })}</p>
          <p className="font-bold text-slate-200 text-sm">{entry.type === EntryType.READING ? `${entry.levelCm}cm` : `${entry.liters}L Delivered`}</p>
        </div>
      </div>
      <button onClick={() => onDelete(entry.id)} className="p-2 text-slate-700 hover:text-red-500 group-hover:opacity-100 transition-opacity"><Trash2 size={14} /></button>
    </div>
  </div>
);
